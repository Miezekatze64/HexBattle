...
public class HexPanel extends JPanel {
    ...
    public void init(String name) {
        ...
        if (!Main.isHost)
            Main.client.setEventListener((e) -> {
                try {
                    if (currentPlayer != null)
                        currentPlayer.newEvent(e);
                    ...
                    case Event.EVENT_START_PLAYER: {
                        ...
                        newPlayer(new Hex(Integer.parseInt(split[0]), Integer.parseInt(split[1]),
                    ...
                    case Event.EVENT_START_PLAYER_END:
                        ...
                        Main.client.sendEvent(new Event(Event.EVENT_ADD_PLAYER,
                                player.getPosition().q + "," + player.getPosition().r + "," + player.getPosition().s));
            ...
            Main.client.setEventListener((e) -> {
                    try {
                        ...
                        case Event.EVENT_JOIN: {
                            ...
                            connected.add(e.getValue());

                            Main.getInstance().updateConnectedList(connected);
                            Main.client
                                    .sendEvent(new Event(Event.EVENT_CONNECTED_CHANGED, String.join(",", connected)));
                            Main.client.sendEvent(new Event(Event.EVENT_START_SEED, map.getSeed() + ""));
                            Main.client.sendEvent(new Event(Event.EVENT_START_PLAYER, player.getPosition().q + ","
                                    + player.getPosition().r + "," + player.getPosition().s));

                            for (int i = 0; i < opponents.size(); i++) {
                                Main.client.sendEvent(new Event(Event.EVENT_START_PLAYER,
                                        opponents.get(i).getPosition().q + "," + opponents.get(i).getPosition().r + ","
                                                + opponents.get(i).getPosition().s));
                            }
    ...
    public void nextTurn(boolean fromEvent) {
        ...
        if (!fromEvent)
            Main.client.sendEvent(new Event(Event.EVENT_END_TURN, ""));
