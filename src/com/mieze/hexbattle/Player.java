...
public class Player {
    ...
    public void newEvent(Event e) {
        ...
        case Event.EVENT_GAME_MOVE: {
            ...
            if (f2.hasCharacter() && f2.getCharacter() instanceof Boat
                    ...
                    if (f.hasCharacter()) {
                        if (f.getCharacter().isFromPlayer(this)) {
                            if (f.getCharacter() instanceof Boat) {
                                ...
                                Hex after = f.getHex();
                                Main.client.sendEvent(new Event(Event.EVENT_GAME_MOVE, before.q + "," + before.r + ","
                        ...
                        if (boat_leave) {
                            ...
                            Hex h = f.getHex();
                            Main.client.sendEvent(new Event(Event.EVENT_GAME_LEAVE_BOAT,
    ...
    public void attack(GameCharacter attacker, GameCharacter target, boolean fromEvent) {
        if (!fromEvent)
            Main.client.sendEvent(new Event(Event.EVENT_GAME_ATTACK,
        ...
        double nextHealth = target.getHealth() - (attack / damage) * attacker.getAttackScore() * 0.8;
    ...
    public void conquerCity(Hex h, boolean fromEvent) {
        ...
        if (!fromEvent)
            Main.client.sendEvent(new Event(Event.EVENT_GAME_CONQUER_CITY, h.q + "," + h.r + "," + h.s));

        player.city_count++;
        if (map.getField(h).getOwner() != null)
            map.getField(h).getOwner().city_count--;
    ...
    public void activate(Hex h) {
        if (!active.contains(h) && !isUnexplored(h) && map.getField(h) != null
                && (!map.getField(h).hasCharacter() || !map.getField(h).getCharacter().isFromPlayer(this)
                        || map.getField(h).getCharacter() instanceof Boat)) {
            active.add(h);
        }
    }
